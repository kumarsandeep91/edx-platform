...
class TestECommerceDashboardViews(SharedModuleStoreTestCase):
    ...
    def test_user_admin_set_course_price(self):
        ...
        response = self.client.post(set_course_price_url, data)
        self.assertIn("CourseMode with the mode slug({mode_slug}) DoesNotExist".format(mode_slug='honor'), response.content)
    ...
    def test_add_coupon(self):
        ...
        self.assertIn("coupon with the coupon code ({code}) added successfully".format(code=data['code']), response.content)
        ...
        data['code'] = 'Vs23Ws4j'
        response = self.client.post(add_coupon_url, data)
        self.assertIn("The code ({code}) that you have tried to define is already in use as a registration code"
                        .format(code=data['code']), response.content)
    ...
    def test_delete_coupon(self):
        ...
        self.assertIn('coupon with the coupon id ({coupon_id}) updated successfully'.format(coupon_id=coupon.id), response.content)

        coupon.is_active = False
        coupon.save()

        response = self.client.post(delete_coupon_url, {'id': coupon.id})
        self.assertIn('coupon with the coupon id ({coupon_id}) is already inactive'.format(coupon_id=coupon.id), response.content)

        response = self.client.post(delete_coupon_url, {'id': 24454})
        self.assertIn('coupon with the coupon id ({coupon_id}) DoesNotExist'.format(coupon_id=24454), response.content)
    ...
    def test_get_coupon_info(self):
        ...
        self.assertIn('coupon with the coupon id ({coupon_id}) DoesNotExist'.format(coupon_id=444444), response.content)
