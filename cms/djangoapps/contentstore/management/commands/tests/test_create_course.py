...
class TestCreateCourse(ModuleStoreTestCase):
    ...
    def test_get_course_with_different_case(self, default_store):
        ...
        Scenario:
            ...
            Verify and that get course from store using same course id but different case is not accessible.
        """
        org = 'Org1'
        number = 'Course1'
        run = 'Run1'
        with self.store.default_store(default_store):
            lowercase_course_id = self.store.make_course_key(org.lower(), number.lower(), run.lower())
